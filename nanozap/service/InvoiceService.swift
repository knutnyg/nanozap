...
class InvoiceService {
    ...
    public func decodeInvoice(payreqString:String) throws -> Invoice {
        do {
            ...
            return Invoice(timestamp: timestamp, ammount: Int(res.numSatoshis), description: res.description_p, expiry: expiry, payreq: payreqString)
        } catch {
    ...
    public func payInvoice(invoice:Invoice) throws -> Bool{
        ...
        do {
            ...
            let _ = try client.sendPaymentSync(request)
            return true
        } catch {
            print("Failed to pay")
            return false
